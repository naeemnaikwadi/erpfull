...
const FileViewer = ({ file, isOpen, onClose }) => {
    ...
    if (isPdf(fileType)) {
      ...
      return (
        <div className="w-full h-full">
          {fileUrl.includes('res.cloudinary.com') || blobUrl ? (
            <iframe
              src={`${(blobUrl || fileUrl)}#toolbar=1&navpanes=1&scrollbar=1&view=FitH`}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className={`bg-white rounded-lg shadow-xl w-full h-full overflow-hidden ${isPdf(file.mimetype || file.fileType) ? 'max-w-7xl max-h-[98vh] flex flex-col' : 'max-w-4xl max-h-[90vh]'}`}>
        {/* Header */}
        ...
        {/* Content */}
        <div className={`${isPdf(file.mimetype || file.fileType) ? 'flex-1' : 'p-4 overflow-y-auto max-h-[calc(90vh-120px)]'}`}>
          {!isPdf(file.mimetype || file.fileType) && renderFileInfo()}
