...
const Profile = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // enrich local user with ERP fields
        login({ ...(user || {}), ...data });
      } catch (e) {
  ...
  const saveProfile = async () => {
    try {
      ...
      // For students, only allow password changes and profile photo
      if (user?.role === 'student') {
        // Check if password is being changed
        if (!formData.newPassword) {
          setError('No changes to save. Only password can be changed.');
          setLoading(false);
          return;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Students can change only password via this endpoint and profile image via upload
      // Name/email edits are blocked for students
      // Only include password fields if changing password
      if (formData.newPassword) {
        updateData.currentPassword = formData.currentPassword;
        updateData.newPassword = formData.newPassword;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.name}
                    disabled={true}
                    className="w-full px-4 py-3 border rounded-lg border-gray-200 bg-gray-50 dark:bg-gray-700 dark:border-gray-600 text-gray-500"
                    required
                  />
                  {user?.role === 'student' && (
                    <p className="text-xs text-gray-500 mt-1">Name cannot be changed. Contact admin for updates.</p>
                  )}
                </div>

                {/* Email Field - Read Only for Students */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  />
                  {user?.role === 'student' && (
                    <p className="text-xs text-gray-500 mt-1">Email cannot be changed. Contact admin for updates.</p>
                  )}
                </div>
              </div>
