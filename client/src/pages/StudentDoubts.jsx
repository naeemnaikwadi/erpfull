  ...
  const fetchDoubts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setPagination(data.pagination || { currentPage: 1, totalPages: 1, totalDoubts: 0 });
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('Failed to fetch doubts:', response.status, errorData);
        setError(`Failed to fetch doubts: ${errorData.message || response.statusText}`);
        setDoubts([]);
      }
    } catch (error) {
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-6 text-sm text-gray-500 dark:text-gray-400 mb-3">
                          <div className="flex items-center gap-1">
                            ...
                            {doubt.course && typeof doubt.course === 'object' ? doubt.course.name : (doubt.course || 'Unknown Course')}
                          </div>
                          <div className="flex items-center gap-1">
                            <User className="w-4 h-4" />
                            {doubt.classroom && typeof doubt.classroom === 'object' ? doubt.classroom.name : (doubt.classroom || 'Unknown Classroom')}
                          </div>
                          <div className="flex items-center gap-1">
                        ...
                        {doubt.answer && (
                          <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                            ...
                            <p className="text-xs text-green-600 dark:text-green-300 mt-2">
                              Answered by {doubt.answer.instructor && typeof doubt.answer.instructor === 'object' ? doubt.answer.instructor.name : (doubt.answer.instructor || 'Unknown')} on {formatDate(doubt.answer.answeredAt)}
                            </p>
                          </div>
                        )}
                      </div>
