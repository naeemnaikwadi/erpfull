  ...
  const handleNextStep = () => {
    if (currentStep < learningPath.steps.length - 1) {
      ...
      const passedPreviously = Array.isArray(userProgress?.quizResults)
        ...
        : false;
      if ((step?.hasQuiz || stepQuiz) && step?.quizRequired && stepQuiz && !((quizResults && quizResults.passed) || passedPreviously)) {
  ...
  const handleCompleteSession = async () => {
    try {
      ...
      const passedPreviously = Array.isArray(userProgress?.quizResults)
        ? userProgress.quizResults.some(r => String(r.stepId) === String(step._id) && r.passed)
        : false;
      if ((step?.hasQuiz || stepQuiz) && step?.quizRequired && stepQuiz && !passedPreviously) {
        setActiveQuiz(stepQuiz);
        return;
      }
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-2">
                    ...
                    {(step?.hasQuiz || stepQuiz) && stepQuiz && (
                      <button
