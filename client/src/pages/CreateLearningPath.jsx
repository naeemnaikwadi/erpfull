  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Create quizzes for steps that have quiz drafts
        const quizCreationPromises = steps
          .map((step, index) => {
            // Find the corresponding step in the created learning path
            const createdStep = learningPathData.steps[index];
            if (!step.quizDraft || !step.quizDraft.questions || step.quizDraft.questions.length === 0) {
              return null;
            }
            ...
            return (async () => {
              try {
                const quizData = {
                  title: step.quizDraft.title,
                  description: step.quizDraft.description,
                  learningPathId: learningPathData._id,
                  stepId: createdStep._id, // Use the step ID from the created learning path
                  timeLimit: step.quizDraft.timeLimit,
                  passingScore: step.quizDraft.passingScore,
                  allowRetakes: step.quizDraft.allowRetakes,
                  maxAttempts: step.quizDraft.maxAttempts,
                  questions: step.quizDraft.questions
                };
                ...
                if (quizResponse.ok) {
                  ...
                  console.error('Failed to create quiz for step:', step.title);
                  return null;
                }
              } catch (error) {
                console.error('Error creating quiz for step:', step.title, error);
                return null;
              }
            })();
          })
        ...
        // Wait for all quiz creations to complete
        await Promise.all(quizCreationPromises);
