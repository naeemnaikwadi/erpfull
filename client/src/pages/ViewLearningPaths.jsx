  ...
  const loadLearningPaths = async () => {
    try {
      ...
      if (response && response.data) {
        // Filter out any paths with null courseId
        const validPaths = response.data.filter(path => path && path.courseId);
        setPaths(validPaths || []);
        console.log('Learning paths loaded:', validPaths);
      } else {
        setPaths([]);
  ...
  const getLearningPathsForCourse = (courseId) => {
    ...
    if (selectedCourseFilter === 'all' || selectedCourseFilter === courseId) {
      return paths.filter(path => {
        if (!path || !path.courseId) return false;
                  ...
                  <div className="p-6">
                    {coursePaths.length > 0 ? (
                      ...
                      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                        <p>No learning paths available for this course yet.</p>
                        {userRole === 'instructor' && (
