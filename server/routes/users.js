...
router.get('/lookup', auth, async (req, res) => {
  try {
    ...
    if (!email && !prn) return res.status(400).json({ message: 'email or prn required' });
    const user = await User.findOne(query).select('name email role avatarUrl prn branch academicYear');
...
// Get basic user info by id (admin and registrar only)
router.get('/:id/basic', auth, async (req, res) => {
  try {
    // Limit to privileged roles
    if (!['admin', 'registrar', 'admission_officer', 'fee_manager', 'hostel_manager', 'exam_controller', 'accountant'].includes(req.user.role)) {
      return res.status(403).json({ message: 'Access denied' });
    }
    const user = await User.findById(req.params.id).select('name email role avatarUrl prn branch academicYear');
    if (!user) return res.status(404).json({ message: 'User not found' });
    res.json(user);
  } catch (e) {
